...
#check that 'normCellHTSobject' is a cellHTS cellHTSobject
	if(class(normCellHTSobject) != "cellHTS") stop("The argument 'normCellHTSobject' is not a cellHTS object")
...
#normalized, but not scored)
	if(state(normCellHTSobject)["configured"] == FALSE) stop("The cellHTS object should be configured to perform the statistical tests")
	if(state(normCellHTSobject)["normalized"] == FALSE) warning("Your cellHTS object has not been normalized, this could impact the results of these tests",immediate.=TRUE)
	if(state(normCellHTSobject)["scored"] == TRUE) stop("This cellHTS object has been scored; the statistical analysis should be performed on the normalized signal intensities",immediate.=TRUE)
	if(state(normCellHTSobject)["annotated"] == FALSE) stop("This cellHTS object has not been annotated",immediate.=TRUE)
...
#check that the annotationColumn matches a column in the fData(cellHTSobject) dataframe	
	if(is.null(match(annotationColumn, colnames(fData(normCellHTSobject))))) stop("The 'annotationColumn' parameter does not match to any column in your cellHTS object")
...
#this cutoff is taken in terms of absolute value
	if(length(cutoffHitsEnrichment) != 1 | (class(cutoffHitsEnrichment) != "integer" && class(cutoffHitsEnrichment) != "numeric")) stop("The cutoffHitsEnrichment should be a single integer ")
