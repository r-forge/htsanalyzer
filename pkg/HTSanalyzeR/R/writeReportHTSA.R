...
#Produce the index html page
	...
	cat('<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"http://www.w3.org/TR/html4/loose.dtd"> \n <html> \n <h1>Report from HTSanalyzeR', file = htmlfile)
	cat(paste('&nbsp &nbsp <img src="./Rlogo.png" width="50" height="40"/>',sep="" ),append = TRUE, file = htmlfile)
	cat(paste('&nbsp <img src="./blue_cruklogo.gif" width="120" height="50"/>',sep="" ),append = TRUE, file = htmlfile)
	cat(paste('&nbsp <img src="./goatcomputer.png" width="110" height="80"/> </h1> <hr/>',sep="" ),append = TRUE, file = htmlfile)
	cat(paste('\n <link rel="stylesheet" type="text/css" href="./htsanalyzer.css" />',sep=""),append = TRUE, file = htmlfile)
	...
	for(i in 1:l.HyperGeo.results){
		...
		cat(paste(names(enrichmentAnalysis$HyperGeo.results)[i],' Hyperg. Tests">',names(enrichmentAnalysis$HyperGeo.results)[i],' Hyperg. Tests</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
		}
	...
	for(i in 1:l.GSEA.results){
		...
		cat(paste(names(enrichmentAnalysis$GSEA.results)[i],' GSEA">',names(enrichmentAnalysis$GSEA.results)[i],' GSEA</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
		}
	...
	for(i in 1:l.Sig.adj.pvals.in.both){
			...
			cat(paste(names(enrichmentAnalysis$Sig.adj.pvals.in.both)[i],' Enrichment.summary">',names(enrichmentAnalysis$Sig.adj.pvals.in.both)[i],' Enrichment.summary</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
			}
	cat("\n </tr> <tr>", append = TRUE, file = htmlfile)
...
#check that the arguments experimentName, geneListName have length one
	...
	cat(paste("\n <br> <br> This report is on the experiment: ",experimentName,sep=""),append = TRUE, file = htmlfile)
...
#check that the arguments cutoffHits, p.adj.method, min.gene.set.size, nPermutations have length one	
	...
	cat("\n \t \t </UL>", append = TRUE, file = htmlfile)
	cat("\n \t </UL>", append = TRUE, file = htmlfile)
	cat("\n <hr/>",append = TRUE, file = htmlfile)
	cat(paste("\n <br> <datestamp>",date()),append = TRUE, file = htmlfile)
...
#Produce the main data frame: headers and first row		
		...
		signif(enrichmentAnalysis$HyperGeo.results[[n]][1,5], digits=4),"</td> \n <td>",
		signif(enrichmentAnalysis$HyperGeo.results[[n]][1,6], digits=4),"</td> \n <td>",
		signif(enrichmentAnalysis$HyperGeo.results[[n]][1,8], digits=4),"</td> \n </tr>", sep=""), append = TRUE, file = htmlfile)
#Produce the main data frame: rows 2 to last		
	for(i in 2:dim(enrichmentAnalysis$HyperGeo.results[[n]])[1]){
	if((i%%2) == 0){
...
#Produce the main dataframe: headers and first row	
		...
		cat(paste('\n <hr/> \n <br>',names(enrichmentAnalysis$GSEA.results)[n],' GSEA ' ,'\n <br> \n <table> <tr> <th>Gene Set name</th> <th>',
			...
			colnames(enrichmentAnalysis$GSEA.results[[n]])[4],"</th> \n <th>",
				"Plots","</th> \n </tr>",
				'<tr class="odd"> <td><a href="http://www.genome.jp/dbget-bin/www_bget?pathway:',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[1],perl=TRUE),'" target="_blank" title="',rownames(enrichmentAnalysis$GSEA.results[[n]])[1],'">',rownames(enrichmentAnalysis$GSEA.results[[n]])[1],'</a> </td> \n <td>',
			signif(enrichmentAnalysis$GSEA.results[[n]][1,1], digits=4),"</td> \n <td>",
			signif(enrichmentAnalysis$GSEA.results[[n]][1,2], digits=4),"</td> \n <td>",
			signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
			signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
...
#Produce the main dataframe: rows with plots	
		for(i in 2:nGseaPlots){
			...
			kegggsname<-sub(pattern="( :: \\w*$)",replacement="",x=kegggsname,perl=TRUE)
			if((i%%2) == 0){
				cat(paste('<tr class="even"> <td><a href="http://www.genome.jp/dbget-bin/www_bget?pathway:',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[i],perl=TRUE),'" target="_blank" title="',rownames(enrichmentAnalysis$GSEA.results[[n]])[i],'">',rownames(enrichmentAnalysis$GSEA.results[[n]])[i],'</a> </td> \n <td>',
				signif(enrichmentAnalysis$GSEA.results[[n]][i,1], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][i,2], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
					...
					cat(paste('<tr class="odd"> <td><a href="http://www.genome.jp/dbget-bin/www_bget?pathway:',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[i],perl=TRUE),'" target="_blank" title="',rownames(enrichmentAnalysis$GSEA.results[[n]])[i],'">',rownames(enrichmentAnalysis$GSEA.results[[n]])[i],'</a> </td> \n <td>',
					signif(enrichmentAnalysis$GSEA.results[[n]][i,1], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][i,2], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
...
#Produce the tabs	
	...
	for(i in 1:l.HyperGeo.results){
		cat(paste('\n <td class="tabs"> <h3><a href="./hyperg',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
		cat(paste(names(enrichmentAnalysis$HyperGeo.results)[i],' Hypergeometric Tests">',names(enrichmentAnalysis$HyperGeo.results)[i],' Hyperg. Tests</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
		}
	cat("\n </tr> <tr>", append = TRUE, file = htmlfile)	
	for(i in 1:l.GSEA.results){
		cat(paste('\n <td class="tabs"> <h3><a href="./gsea',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
...
#Produce the networks analysis results page
	...
	cat(paste('\n <link rel="stylesheet" type="text/css" href="./htsanalyzer.css" />',sep=""),append = TRUE, file = htmlfile)
#Produce the tabs	
	cat(paste('\n <table class="noframe"> <tr> <td class="tabs"> <h3><a href="./index.html" title="gsea">Index</a></h3> </td> </tr>',sep=""), append = TRUE, file = htmlfile)
	cat("\n <tr>", append = TRUE, file = htmlfile)
	for(i in 1:l.HyperGeo.results){
		cat(paste('\n <td class="tabs"> <h3><a href="./hyperg',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
		cat(paste(names(enrichmentAnalysis$HyperGeo.results)[i],' Hypergeometric Tests">',names(enrichmentAnalysis$HyperGeo.results)[i],' Hyperg. Tests</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
	...
	cat("\n </tr> </table>", append = TRUE, file = htmlfile)
...
#Check that the nwAnalysisOutput has the right format
	...
	cat(paste('\n <table class="noframe"> <tr> <td> <table> <tr class="head"> \n <th>',
